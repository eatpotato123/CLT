MODULE DECLARE
    integer,parameter::mxt=256,mzt=256,myt=32,n=200,np=83,nnn=1000
	    ...
	    do i=5,mxt-5,3
		    ...
		    do nn=1,myt*nnn*n,1
			    ...
			    if(mod(nn,myt*nnn)==1)  then
				    times=nn/(myt*nnn)
				    pointr((i-5)/3+1,loc*n+times+1)=r0
				    pointz((i-5)/3+1,loc*n+times+1)=z0
			    endif

			    !判断是否已经绕行半周
			    if(mod(nn,myt*nnn)==1)  then
			        halfs=nn/(myt*nnn)
			        pointr1((i-5)/3+1,loc*n+halfs+1,1)=r0
			        pointz1((i-5)/3+1,loc*n+halfs+1,1)=z0
			    endif


			    if(mod(nn,myt*nnn)==myt*nnn/8+1)  then
			        halfs=nn/(myt*nnn)
			        pointr1((i-5)/3+1,loc*n+halfs+1,2)=r0
			        pointz1((i-5)/3+1,loc*n+halfs+1,2)=z0
			    endif

			    if(mod(nn,myt*nnn)==myt*nnn/4+1)  then
			        halfs=nn/(myt*nnn)
			        pointr1((i-5)/3+1,loc*n+halfs+1,3)=r0
			        pointz1((i-5)/3+1,loc*n+halfs+1,3)=z0
			    endif

			    if(mod(nn,myt*nnn)==myt*nnn/2+1)  then
			        halfs=nn/(myt*nnn)
			        pointr1((i-5)/3+1,loc*n+halfs+1,4)=r0
			        pointz1((i-5)/3+1,loc*n+halfs+1,4)=z0
			    endif
    	    ...
    	    if(z0==0)   then
    	        np00((i-5)/3+1)=np00((i-5)/3+1)+1
    	        nt00((i-5)/3+1)=nn/(myt*nnn)
    	    endif
    	    if(z0*z1<0) then
    	        np00((i-5)/3+1)=np00((i-5)/3+1)+1
    	        nt00((i-5)/3+1)=nn/(myt*nnn)
