...
!boundary conditions respectively in x and z direction
	...
	implicit none
	integer m_times,n_list1,n_list2,n_list3,n_list4
      include 'mpif.h'
...
! fixed boundary
	...
	! update host(x1(ix_last-3:ix_last-2,:,:,:),x1(ix_first+2:ix_first+3,:,:,:))
	! update host(x1(:,iz_last-3:iz_last-2,:,:),x1(:,iz_first+2:iz_first+3,:,:))
	! update host(x1(:,:,iy_last-3:iy_last-2,:),x1(:,:,iy_first+2:iy_first+3,:))
!      call mpi_transfersm(x1(:,:,:,:),8) 
	! update device(x1(ix_last-1:ix_last,:,:,:),x1(ix_first:ix_first+1,:,:,:))
	! update device(x1(:,iz_last-1:iz_last,:,:),x1(:,iz_first:iz_first+1,:,:))
	! update device(x1(:,:,iy_last-1:iy_last,:),x1(:,:,iy_first:iy_first+1,:))


      call mpi_transfer8_x1_openacc
...
!	call smth_irpt_with_difc_v3(2,1,1,8,0.85d0) !open if necessary
   ...
   19 continue
	...
	do 20 m=1,8
      ...
      x(jx,jz,jy,m)=x1(jx,jz,jy,m)+xint(jx,jz,m)
   20 continue
	!$acc end kernels
	endif
...
! fixed boundary
	...
	do jy=iy_first,iy_last
	do jx=ix_first,ix_last
	x_8bndx(jx,jy,m)=x1_8bndx(jx,jy,m)+xint_8bndx(jx,m)
	x_8bndz(jx,jy,m)=x1_8bndz(jx,jy,m)+xint_8bndz(jx,m)
	enddo
	enddo
	enddo
	!$acc end kernels
...
! send x1 down unless i'm at the bottom
...
!mpi   ----------------------------------------------------------------
	...
	!$acc kernels present(x1,wfx1,wfx2,wfy1,wfy2,wfz1,wfz2)
      x1(:,:,iy_first+1,:)=x1(:,:,iy_last-2,:)
      x1(:,:,iy_first,:)=x1(:,:,iy_last-3,:)
      x1(:,:,iy_last-1,:)=x1(:,:,iy_first+2,:)
      x1(:,:,iy_last,:)=x1(:,:,iy_first+3,:)
	!$acc end kernels
      endif
	! end data
...
! allocate the rvx_bndx, rvx_bndz, rey_bndx, rey_bndz for rvx points	
	allocate(rvx_bndx(nbndx,my))
	allocate(rey_bndx(nbndx,my))
	allocate(rvx_bndz(nbndz,my))
	allocate(rey_bndz(nbndz,my))
