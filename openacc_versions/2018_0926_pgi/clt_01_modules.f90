...
module declare_parameter
      integer, parameter :: mxt=128,myt=64,mzt=128,npsi=111,nthe=101,mbm=4*mxt+4*mzt,mr=4,mdgn=1
...
!gdtp_ep(:,:,4-6): grd_type(:,:,2) and gdtp_bndz(:,:,1-2)	
	...
	!$acc declare create(bndx_grd,bndz_grd)
	...
	!$acc declare create(x_8bndz,x_8bndx,x1_8bndz,x1_8bndx,ef_3bndx,ef_3bndz,xint8_bndz,xint_8bndx,xint_8bndz)

	real*8, allocatable :: b_rmp_bndx(:,:,:), b_rmp_bndz(:,:,:), b_rmp_bndx_tmp1(:,:,:), b_rmp_bndz_tmp1(:,:,:), &
		  b_rmp_bndx_tmp2(:,:,:), b_rmp_bndz_tmp2(:,:,:)
	!$acc declare create(b_rmp_bndx,b_rmp_bndz)

      real*8, allocatable :: xy_8bndx(:,:,:),xy2_8bndx(:,:,:)
	!$acc declare create(xy_8bndx,xy2_8bndx)
      real*8, allocatable :: xy_8bndz(:,:,:),xy2_8bndz(:,:,:)
	!$acc declare create(xy_8bndz,xy2_8bndz)
...
module declare_grid
	...
	!$acc declare create(yy)
      real*8, dimension(mx) :: xx,dx
	!$acc declare create(xx)
...
module declare_for_openacc
	...
	!$acc declare create(rvx,rey, drvx_dx_tmp, dex_dy_tmp, dez_dy_tmp)
!	real*8, dimension(nbndx,my) :: rvx_bndx, rey_bndx
	real*8, allocatable :: rvx_bndx(:,:), rey_bndx(:,:)
	!$acc declare create(rvx_bndx,rey_bndx)
!	real*8, dimension(nbndz,my) :: rvx_bndz, rey_bndz
	real*8, allocatable :: rvx_bndz(:,:), rey_bndz(:,:)
	!$acc declare create(rvx_bndz,rey_bndz)
!	real*8 d1fc, d1f2, d1fm, d1fp, d1fbp ,d1fbm, d1xf2
!     real*8 d1f2m, d1f2p, d2fc
	...
	!$acc declare create(jxjz_list1_bndry)
	!$acc declare create(jxjz_list2_bndry)
	!$acc declare create(jxjz_list3_bndry)
	!$acc declare create(jxjz_list4_bndry)
	...
	!$acc declare create(rby)

      real*8, dimension(my) :: wwy 
	!$acc declare create(wwy)
      real*8, dimension(mx,mz,my) :: x1r_tmp,xr2_tmp,x1z_tmp,xz2_tmp,x1_tmp
	!$acc declare create(x1r_tmp,xr2_tmp,x1z_tmp,xz2_tmp,x1_tmp)
!      real*8 d2f2, d2fbp, d2fbm, timestart1, timeend1

end module declare_for_openacc



module diff_for_openacc
	contains
	real*8 function d1fc(fm2,fm1,f0,fp1,fp2,a,b,c,d)
		  !$acc routine seq
		  real*8, intent(in) :: fm2,fm1,f0,fp1,fp2,a,b,c,d
		  d1fc=a*(fp1-f0)+b*(f0-fm1)+c*(fp2-f0)+d*(f0-fm2)
	end function

	real*8 function d2fc(fm2,fm1,f0,fp1,fp2,a,b,c,d)
		  !$acc routine seq
		  real*8, intent(in) :: fm2,fm1,f0,fp1,fp2,a,b,c,d
		  d2fc=a*(fp1-f0)+b*(f0-fm1)+c*(fp2-f0)+d*(f0-fm2)
	end function

	real*8 function d1fbp(fp2,fp1,f0,fm1,a,b,c)
		  !$acc routine seq
		  real*8, intent(in) :: fm1,f0,fp1,fp2,a,b,c
		  d1fbp=a*(fm1-f0)+b*(fp1-f0)+c*(fp2-f0)
	end function

	real*8 function d2fbp(fm1,f0,fp1,fp2,a,b,c)
		  !$acc routine seq
		  real*8, intent(in) :: fm1,f0,fp1,fp2,a,b,c
		  d2fbp=2*(a*(fm1-f0)+b*(fp1-f0)+c*(fp2-f0))
	end function

	real*8 function d1fbm(fm2,fm1,f0,fp1,a,b,c)
		  !$acc routine seq
		  real*8, intent(in) :: fm2,fm1,f0,fp1,a,b,c
		  d1fbm=a*(f0-fp1)+b*(f0-fm1)+c*(f0-fm2)
	end function

	real*8 function d2fbm(fm2,fm1,f0,fp1,a,b,c)
		  !$acc routine seq
		  real*8, intent(in) :: fm2,fm1,f0,fp1,a,b,c
		  d2fbm=2*(a*(fp1-f0)+b*(fm1-f0)+c*(fm2-f0))
	end function

end module diff_for_openacc
...
!      integer nmode,mmode
	...
	!$acc declare create(fmu,fmux,fmuz)
      real*8, dimension(mx,mz) ::pmu,pmux,pmuz
	!$acc declare create(pmu,pmux,pmuz)
      real*8, dimension(mx,mz) ::kap,kapx,kapz
	!$acc declare create(kap,kapx,kapz)
...
!      integer, dimension(4*mbm) :: jxa1,jza1,it_a1
      ...
      !$acc declare create(ax1,bx1,cx1,dx1)
      real*8, dimension(mx) :: ax2,bx2,cx2,dx2
      !$acc declare create(ax2,bx2,cx2,dx2)
      ...
      !$acc declare create(az1,bz1,cz1,dz1)
      real*8, dimension(mz) :: az2,bz2,cz2,dz2
      !$acc declare create(az2,bz2,cz2,dz2)
	...
	!$acc declare create(ay1,by1,cy1,dy1)
      real*8, dimension(my) :: ay2,by2,cy2,dy2
	!$acc declare create(ay2,by2,cy2,dy2)
