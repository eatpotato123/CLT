...
!      print*, 'success 21719', nstep
      call convt_cut_cell
!      print*, 'success 21721', nstep
      call current_cut_cell
!      print*, 'success 21723', nstep
!      call convtdivb
...
!     call bndry
!      print*, 'success 22422', nstep
      if(invaryrho) xdif(:,:,:,1)=0
      if(invaryp)   xdif(:,:,:,2)=0
      call mpi_transfersm(xdif,8)

!      print*, 'success 22427', nstep
...
! for the xy_8bndx and xy2_8bndx and xy_8bndz and xy2_8bndz
   ...
   19 continue 
    ...
    if((nrank.eq.0).and.(nstep.eq.0)) timestart1=mpi_wtime()
      !execute
      !$ACC PARALLEL LOOP COPYIN(x1_tmp,ax1,bx1,cx1,dx1,az1,bz1,cz1,dz1,ax2,bx2,cx2,dx2,az2,bz2,cz2,dz2,iy_first,iy_last,iz_first,iz_last,ix_first,ix_last) COPYOUT(x1r_tmp,xr2_tmp,x1z_tmp,xz2_tmp) LOCAL(jx,jz,i) annotate(entire(ax1,bx1,cx1,dx1,ax2,bx2,cx2,dx2,az1,bz1,cz1,dz1,az2,bz2,cz2,dz2))
      do jy=iy_first,iy_last
      do jz=iz_first+2,iz_last-2
      do jx=ix_first+2,ix_last-2
!      do i=1,1000000

     x1r_tmp(jx,jz,jy)=d1fc(x1_tmp(jx-2,jz,jy),x1_tmp(jx-1,jz,jy),x1_tmp(jx,jz,jy) &
         ,x1_tmp(jx+1,jz,jy),x1_tmp(jx+2,jz,jy),ax1(jx),bx1(jx),cx1(jx),dx1(jx))
     xr2_tmp(jx,jz,jy)=d2fc(x1_tmp(jx-2,jz,jy),x1_tmp(jx-1,jz,jy),x1_tmp(jx,jz,jy) &
         ,x1_tmp(jx+1,jz,jy),x1_tmp(jx+2,jz,jy),ax2(jx),bx2(jx),cx2(jx),dx2(jx))
     x1z_tmp(jx,jz,jy)=d1fc(x1_tmp(jx,jz-2,jy),x1_tmp(jx,jz-1,jy),x1_tmp(jx,jz,jy) &
         ,x1_tmp(jx,jz+1,jy),x1_tmp(jx,jz+2,jy),az1(jz),bz1(jz),cz1(jz),dz1(jz))
     xz2_tmp(jx,jz,jy)=d2fc(x1_tmp(jx,jz-2,jy),x1_tmp(jx,jz-1,jy),x1_tmp(jx,jz,jy) &
         ,x1_tmp(jx,jz+1,jy),x1_tmp(jx,jz+2,jy),az2(jz),bz2(jz),cz2(jz),dz2(jz))
!      enddo
      enddo
      enddo
      enddo
      ...
      if((nrank.eq.0).and.(nstep.eq.0)) then
      timeend1=mpi_wtime()
      write(*,*) 'run time1=',timeend1-timestart1,nrank,nstep
      endif
...
!      print*, 'success 22981', nstep
      call mpi_transfersm(x1(:,:,:,:),8) 
!      print*, 'success 22983', nstep
